...
const Styledfield = ({gen,userdetails}) => {
  ...
  const handleSubmit = (event) => {
    ...
    const decokedToken = jwt.decode(token);
    setusers(decokedToken.user_id);
    ...
    formData.append('user',userdetails.id);


    axios.post(`http://127.0.0.1:8000/media/${endpoint}/`, formData, { headers: { Authorization: `Bearer ${token}`, }})
      .then((response) => {
  ...
  <Box px={2} sx={{ backgroundColor: checkboy?alpha('#5897e0', 0.9) :alpha('#e167e6', 0.7),overflow:'auto',height:'70vh' }}>
    {/* <form> */}
      ...
      <Grid item xs={12} sm={6}>
        <FormControl fullWidth>
          ...
          <Select
            ...
            value={payload.spous} // make sure value is an array
            // onChange={handleChange}
...
export default withAuth(Styledfield);
