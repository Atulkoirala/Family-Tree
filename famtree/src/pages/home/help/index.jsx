...
import {Box,Grid,Toolbar,Tooltip, TextField,Tab , Tabs , Select, IconButton, Menu, MenuItem,MenuIcon,Typography,Input, Button, FormControl, InputLabel } from '@mui/material';
        ...
        <Layout>
            <motion.Box
                    ...
                    <Box my={2} px={2} whileHover={{opacity:1}}>
                      ...
                      <Box sx={{ height:'100%',overflow:'hidden',borderRadius:'5%',fontWeight:'bold'}}>
                        <Tabs
                          value={value}
                          sx={{width:'100'}}
                    ...
                    {post.map((elem) => {
                      return (
                          <Tab label={elem.Name} key={elem.id} sx={{width:'25%',overflowX:'auto',fontSize:{xl:'1rem',lg:'0.9rem',md:'0.8rem',sm:'0.7rem',xs:'0.68rem'},fontWeight:'bolder'}}/>
                          )
                    })}
                        </Tabs>
                    ...
                    post.map((elem) => {
                      ...
                      <AnimatePresence initial={false} value={value} index={elem.id} key={elem.id} dir={theme.direction} sx={{overflow:'hidden'}} >
                        ...
                        <Box sx={{marginTop:'0.7rem', justifyContent:'center',textAlign:'center',alignItems:'center'
                          ...
                          <Box p={1} sx={{overflow:'auto',maxHeight: '70vh'}}>
