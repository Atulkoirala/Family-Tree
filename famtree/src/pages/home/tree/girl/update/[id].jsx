...
export default withAuth(function GirlId({ id }) {
      ...
      React.useEffect(() => {
        const fetchMediaData = async () => {
          try {
            ...
            const res = await axios.get(`http://127.0.0.1:8000/media/girl/${id}/`,{ headers: { Authorization: `Bearer ${token}`, }});
    ...
    const handleSubmit = (event) => {
      ...
      const token = localStorage.getItem('accessToken');
      axios.put(`http://127.0.0.1:8000/media/gupdate/${id}/`, formData, { headers: { Authorization: `Bearer ${token}`, }})
      .then((response) => {
        ...
        .catch((error) => {
          if (
            ...
            errorHandler("Please select a different Mother. You cannot select the Yourself as Mother.");
          } else {
    ...
    const handleChange = (event) => {
      ...
      // if (value === "UnMarried") {
      //   data.spous = []; // Set an empty array for unmarried status
      //   console.log(data.spous);
      // }
      // }
      // else if(name === "spous"){
      //   setSel(newValue);
            ...
            <Layout>
                        ...
                        <Grid item xs={12} sm={6}>
                          <FormControl fullWidth>
                            ...
                            <Select
                              ...
                              onChange={handleChange}
                              inputLabelProps={{ shrink: true }} // Display the label above the value
                              inputProps={{ autoFocus: true, min: 0 }} // Automatically focus on the input and set min value
                              required
                              fullWidth
                            >
                ...
                <Grid item xs={12} sm={6}>
                    ...
                    <Select
                        ...
                        value={data.dads}
                        onChange={handleChange}
                        helperText="You need to add this again before submiting"
                    >
                ...
                <Grid item xs={12} sm={6}>
                    ...
                    <Grid item xs={12} sm={12} sx={{ display: 'inline', justifyContent: 'center',textAlign:'center',borderBottom:'1px solid black',width:'100%'}} py={1} ml={1.5}>
                            <Input
                                ...
                                {
                                (data.image || prof) ? (
                                  <Box my={2} sx={{border:'1rem solid #FF1493',margin:'auto',borderRadius:'50%',width:'fit-content',justifyContent:'center',alignItems:'center',textAlign:'center',background:'#FF1493'}}>
                                    <Image src={prof || data.image} width={220} height={220} style={{ borderRadius: '50%' }} alt="Image" />
                                  </Box>
                                ) : (
                                    <></>
                                )
