...
export default withAuth(function TreeBoy({ treeid }) {
  ...
  const updateClick = ()=>{
    router.push('/home/tree/boy/update/'+treeid);
  }
  ...
  React.useEffect(() => {
    const fetchMediaData = async () => {
      try {
        ...
        const res = await axios.get(`http://127.0.0.1:8000/media/boy/${treeid}/`, { headers: { Authorization: `Bearer ${token}`, }});
        setpost(res.data);
      } catch (error) {
        console.log(error);
      }
  ...
  const rows=[
    ...
    : []),
    { id: 11, name: 'Father', Description: post.pname ?? 'null' },
    { id: 12, name: 'Mother', Description: post.Mother ?? 'null' },
    {id:13 , name : 'Marrital Status' , Description: post.Marrital_Status },
    ...(post.Marrital_Status === 'Married'
    ? [{ id: 14, name: 'Spouse', Description: post.Wife.length > 1 ? post.Wife.join(" , ") : post.Wife }]
    : []),
    ...(post.Marrital_Status === 'Married'
    ? [{
      id: 15,
        ...
        <Box sx={{display:'flex',justifyContent:'center',textAlign:'center',alignItems:'center'}}>
          ...
          {post.spous !== null && Array.isArray(post.spous) && post.spous.length > 1 ? (
            post.spous.map((spouse, index) => (
              <Children key={index} pks={spouse} gen="Female" col="skyblue" />
            ))
          ) : (
            <Children pks={post.spous} gen="Female" col="skyblue" />
          )}
        </Box>
      ),
    }]
                ...
                <Grid item xs={12} p={1}>
                  <Grid container sx={{overflowX:'auto'}}>
                    <Grid item xs={7} sm={5} my={1} mx={2} display="block">
                        ...
                        <Box sx={{border:'1rem solid #87CEEB',margin:'auto',borderRadius:'50%',width:'fit-content',justifyContent:'center',alignItems:'center',textAlign:'center',background:'#87CEEB'}}>
                      ...
                      <TableBody>
                        {rows.map((row) => (
                          <TableRow key={row.name} sx={{
                            '&:nth-of-type(odd)': {
                              background: 'linear-gradient(rgba(128, 128, 128, 0.8),  rgba(135, 206, 250, 0.5))',
                            },
                            '&:nth-of-type(even)': {
